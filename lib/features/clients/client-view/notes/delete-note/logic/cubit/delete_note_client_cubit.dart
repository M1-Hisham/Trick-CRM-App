import 'package:trick_crm_app/core/api/api_service.dart';
import 'package:trick_crm_app/core/cubits/base_cubit.dart';
import 'package:trick_crm_app/core/models/feat_status_model.dart';
import 'package:trick_crm_app/core/repo/base_repo.dart';

class DeleteNoteClientCubit extends BaseCubit<FeatStatusModel> {
  DeleteNoteClientCubit(ApiService apiService)
      : super(
          BaseRepo(
            fetchData: ({Map<String, dynamic>? params}) =>
                apiService.deleteClientNote(params!['id'], params['idNote']),
          ),
        );
  Future<void> deleteClientNote(int clientId, int noteId) async {
    await getData(
      params: {'id': clientId, 'idNote': noteId},
    );
  }
}
